...
import { AppConfig, getCookie } from '@/config'
...
export default function ChatContainer() {
  ...
  const handleStreamMode = async (content: string) => {
    try {
      ...
      // 调用流式API
      // fetch 可以直接返回流式数据
      const response = await fetch(AppConfig.apiBaseUrl + '/api/agent/stream', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-Time': new Date().toLocaleString(),
          ...
          if (line.startsWith('data: ')) {
            ...
            try {
              ...
              if (parsed.content) {
                ...
                // 更新现有消息
                setMessages((prev) =>
                  prev.map((msg) => (msg.id === placeholderId ? { ...msg, content: accumulatedContent } : msg))
                )
              }
            } catch (e) {
        ...
        <div className='space-y-6'>
          {messages.length === 0 ? (
            ...
            <div className='text-center py-12'>
              ...
              <p className='text-muted-foreground'>我可以帮助你解答问题、创作内容、编程协助等。请开始对话吧！</p>
            </div>
          ) : (
          ...
          {isLoading && !isStreamMode && <MessageBubble content='' role='assistant' isLoading={true} />}
          {/* 错误提示 */}
          {/* {error && (
